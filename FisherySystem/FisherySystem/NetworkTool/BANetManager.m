...
+ (BAURLSessionTask *)ba_requestWithType:(BAHttpRequestType)type
    ...
    BAWeak;
    /*! 检查地址中是否有中文 */
    NSString *baseUrl = [NSURL URLWithString:entity.urlString] ? entity.urlString : [self strUTF8Encoding:entity.urlString];
    NSString *URLString = [NSString stringWithFormat:@"%@%@",[RequestConstants baseURL],baseUrl];
...
+ (void)ba_uploadVideoWithEntity:(BADataEntity *)entity
    ...
    /*! 获得视频资源 */
    // NSLog(@"获得视频资源 = %@",fileEntity.filePath);
    // file:///private/var/mobile/Containers/Data/Application/02391C5E-6062-4C96-90B0-D4606B73A580/tmp/68189411815__171EA3EE-4162-437B-A21A-974CAB2E82CB.MOV
    AVURLAsset *avAsset = [AVURLAsset URLAssetWithURL:fileEntity.filePath  options:nil];
    ...
    NSURL *outputURL;
    outputURL = avAssetExport.outputURL; //默认.MOV
    
    [avAssetExport exportAsynchronouslyWithCompletionHandler:^{
        switch ([avAssetExport status]) {
            case AVAssetExportSessionStatusCompleted: {
                
                NSLog(@"视频长度 = %@",[NSString stringWithFormat:@"%.2fs", [BANetManager getVideoLength:outputURL]]);
                NSLog(@"文件大小 = %@", [NSString stringWithFormat:@"%.2fKB", [BANetManager getFileSize:[outputURL path]]]);
                NSLog(@"所上传文件的路径 = %@",outputURL);
                // file:///var/mobile/Containers/Data/Application/B655717B-433F-42BE-82DC-B1E2BF0D735F/Documents/output-2022-08-11-15:06:21.mp4
                
                dispatch_async(dispatch_get_main_queue(), ^{
                    [MyProgressHUD showWithStatus:@"正在上传..."];
                });
...
//视频上传（4）此方法是对视频上传条件进行限制。
+ (void)alertUploadVideo:(NSURL*)URL{
    CGFloat size = [BANetManager getFileSize:[URL path]];
    NSString *message;
    NSString *sizeString;
    ...
    NSLog(@"检测视频时长 size = %.2f",sizemb);
    NSLog(@"检测视频时长 = %@",sizeString);
...
//        [self uploadVideo:URL];
        ...
        message = [NSString stringWithFormat:@"当前视频时长%@，超过60MB最大限制，不能上传，抱歉。", sizeString];
        /*! 主线程刷新UI */
        dispatch_async(dispatch_get_main_queue(), ^{
