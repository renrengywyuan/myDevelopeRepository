...
- (LEEConfigToStringAndBlock)LeeAction{
    ...
    return ^(NSString *title , void(^block)(void)){
        ...
        return weakSelf.LeeAddAction(^(LEEAction *action) {
            ...
            action.backgroundColor = RGB(74, 120, 237);
...
- (LEEConfigToStringAndBlock)LeeCancelAction{
    ...
    return ^(NSString *title , void(^block)(void)){
        ...
        return weakSelf.LeeAddAction(^(LEEAction *action) {
            ...
            action.font = FontRegular(15);
            
            action.backgroundColor = [UIColor colorWithHexString:@"#F4F4F4"];
...
- (void)configAlert{
    ...
    for (id item in self.config.modelActionArray) {
        ...
        if (!action.font) action.font = FontMedium(15);
        
        if (!action.title) action.title = @"按钮";
        
        if (!action.titleColor) action.titleColor = [UIColor colorWithHexString:@"#333333"];
        ...
        if (!action.borderColor) action.borderColor = [UIColor colorWithHexString:@"#EBEBEB"];
...
- (void)configActionSheet{
    ...
    for (id item in self.config.modelActionArray) {
        ...
        if (!action.font) action.font = FontRegular(18);
    ...
    switch (type) {
        ...
        case LEEAlertTypeActionSheet:
            ...
            .LeeConfigMaxWidth(^CGFloat(LEEScreenOrientationType type) {
                ...
                return type == LEEScreenOrientationTypeHorizontal ? KScreenH - 20.0f : KScreenW - 20.0f;
            })
            .LeeConfigMaxHeight(^CGFloat(LEEScreenOrientationType type) {
